  ...
  \begin{equation}
    \begin{split}
      ...
      & + C_4(T,Re,Ro,F,\Gamma_1,\Gamma_2,\Gamma_4, \int_{0}^{T}\!\|\Delta
        \psi\|^4\, dt)\,\|\Delta(\psi - \lambda^h)\|_{L^4(0,T;L^2)} \\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    & + \|\nabla \left(\psi - \lambda^h\right)(T)\|^2.
  \end{align*}
  Finally, taking $\inf_{\lambda^h \in X^h}$ of both sides and letting
  \begin{align*}
  ...
  \begin{align*}
    ...
    & + \inf_{\lambda^h \in X^h} \biggl\{C_2(T,Re,\Gamma_3)
      \int_0^T\! \|\nabla \left( \psi - \lambda^h\right)_t\|_{-1}^2
      + C_3(Re,Ro,\Gamma_2)\,\|\Delta \left(\psi - \lambda^h\right)\|^2\, dt \\
    & + C_4(T,F,\psi_0,Re,Ro,\Gamma_1,\Gamma_3,\Gamma_4,\int_{0}^{T}\!\|\Delta
      \psi\|^4\, dt)\, \|\Delta \left(\psi - \lambda^h\right)\|^2_{L^4(0,T;L^2)} \\
    & + 2\, \|\nabla \left(\psi - \lambda^h\right)(T)\|^2\biggr\},
  \end{align*}
  which is the desired result.
\end{proof}

{\color{red} {\large Are these the correct rates? Are we in the correct space? Can we
get another power of $h$ here?} \\
The following lemma will allow for the determination of FE error bounds in the
term involving $\frac{\partial}{\partial t}$.
\begin{lemma} \label{lma:Interpolation}
  Suppose the nodes $N_j$ of the finite element mesh do not move. Let $X^h
  \subset C^1 \bigcap X$, consisting of Argyris elements. Then
  \begin{align}
    ...
    \left\|\frac{\partial \left[\nabla \psi\right]}{\partial t}
      ...
      &\le C\, h^4\, \left|\frac{\partial \psi}{\partial t}\right|_5,
      \text{ and } \label{eqn:dH1} \\
    \left\|\frac{\partial \left[\nabla \psi\right]}{\partial t}
      - \frac{\partial}{\partial t} \nabla \left(I^h \psi\right)\right\|_{-1}
      &\le C\, h^4\, \left|\frac{\partial \psi}{\partial t}\right|_5. \label{eqn:dH-1}
  \end{align}
\end{lemma}
\begin{proof}
  Equation \eqref{eqn:dInterp} follows from the explicit formulas for the linear
  interpolant, equation \eqref{eqn:dH1} follows from a combination of
  \eqref{eqn:dInterp} and \eqref{eqn:ArgyrisBound} with $p=2,q=2,m=1,$ and
  $k=4$. Finally, equation \eqref{eqn:dH-1} follows from the fact that
  $\|\cdot\|_{-1} \le \|\cdot\|$.
\end{proof}

\begin{lemma} \label{lma:IntegralErrors}
  Let $X^h$ denote the finite element space associated with the Argyris element.
  Suppose the interpolation estimates from \autoref{lma:Interpolation} hold and
  that $\psi, \psi_t \in C^1(\Omega \times [0,T])$. Then,
  \begin{equation}
    \int_{0}^{T}\! \|\nabla \left(\psi - \lambda^h\right)_t\|_{-1}^2 + \|\Delta
    \left(\psi - \lambda^h\right) \|^2\, dt \le C\, h^8
    \int_{0}^{T}\! \,|\psi_t|_5^2 + |\psi|_6^2\, dt
    \label{eqn:IntegralErrors}
  \end{equation}
  and
  \begin{equation}
    \|\Delta\left( \psi- I^h \psi\right)\|_{L^4(0,T; L^2(\Omega)}^2 \le C h^8
      |\psi|_{L^4(0,T;H^6(\Omega)}). \label{eqn:L4Interpolation}
  \end{equation}
\end{lemma}
\begin{proof}
  At each time step we see from inequality (6.1.5) in \cite{Ciarlet} that
  $\|\Delta\left(\psi - \psi\right)\| \le C\, h^4\, |\psi|_6$. Squaring and
  ...
  \begin{align*}
    ...
    &= \left(\int_{0}^{T}\! \|\Delta\left(\psi - I^h \psi\right)\|^4\, dt
      \right)^{\frac{1}{2}} \\
    &\le \left(\int_{0}^{T}\! C\, h^4 |\psi|_6\, dt\right)^{\frac{1}{2}},
  \end{align*}
  which leads to the desired estimate $\|\Delta\left(\psi -
  I^h\right)\|_{L^4(0,T;L^2(\Omega))} \le C\, h^8
  \|\psi\|_{L^4(0,T;H^6(\Omega))}$.
\end{proof}

\begin{thm} \label{thm:SemiInterp}
  Let $X^h$ be the FE space associated with the Argyris element and let $I^h$
  denote the interpolant for the Argyris element. Suppose the interpolation
  estimates \autoref{lma:Interpolation} in $H^{-1}(\Omega)$ hold and that $\psi,
  \psi_t \in C^1(\Omega\times [0,T])$.  Suppose also that the assumptions of
  \autoref{thm:StrongConvergence} hold.  Then,
  \begin{equation}
    \begin{split}
      \|\nabla \left( \psi - \psi^h\right)(T) \|^2
        &+ Re^{-1} \int_0^T\! \|\Delta \left(\psi - \psi^h\right)\|^2\, dt
        \le h^8\, C\, \biggl\{
        C_1(T,Re,\Gamma_3) {\color{blue} |\psi|_5^2} \\
      & + C_2(T,Re,\Gamma_3) \left( \|\psi_t\|_{L^2(0,T;H^5(\Omega))}
        + C_3(Re,Ro,\Gamma_2)\,\|\psi\|_{L^2(0,T;H^6(\Omega))}\right) \\
      & + C_4(T,F,\psi_0,Re,Ro,\Gamma_1,\Gamma_3,\Gamma_4,\int_{0}^{T}\!\|
        \psi\|^4\, dt)\, \|\psi\|_{L^4(0,T;H^6(\Omega))} \\
      & + 2\,{\color{blue} |\psi\|^2_5}\biggr\},
    \end{split}
    \label{eqn:SemiInterp}
  \end{equation}
\end{thm}
\begin{proof}
  {\color{blue} \Large Is this correct? Or should something else be done with
them?}\\
  The proof is a combination of the previous two lemmas.
\end{proof}
}
