One of the most popular mathematical models used in the study of large scale wind-driven ocean
...
As noted in Chapter 10.7.2 in \cite{Vallis06} (see also \cite{San12}), the eddy viscosity parameter
...
We first need to choose a length scale and a velocity scale. The length scale we choose is $L$, the
...
Next, we nondimensionalize \eqref{qge_q_psi_dim_1}. We start with the left-hand side:
\begin{align}
...
Next, we nondimensionalize the right-hand side of \eqref{qge_q_psi_dim_1}. The first term can be
...
\end{align}
As noted above, the Sverdrup balance expresses the balance between the two dominant effects in the
system: the $\beta$-effect and the curl of the divergence of the wind stress (see, e.g., Section
14.1.3 in \cite{Vallis06}).  The velocity scale $U$ in \eqref{eqn:velocity_scale} was chosen
according to the Sverdrup balance.  Thus, the last term on the right-hand side of
...
\end{align}
which, after the obvious simplification, is clearly nondimensional.
Thus, \eqref{eqn:qge_q_psi_nondim_11} clearly shows that the last term on the right-hand side of \eqref{eqn:qge_q_psi_nondim_10} is nondimensional, so \eqref{eqn:qge_q_psi_nondim_10} becomes:
\begin{align}
...
Dropping the $^*$ superscript in \eqref{eqn:qge_q_psi_nondim_12}, we obtain the nondimensional {\it
...
%\textcolor{blue}{Traian: Make sure you define $Re$ and $Ro$.}
...
Thus, for notation consistency (i.e., to ensure that the velocity and the streamfunction are related
