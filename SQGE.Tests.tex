...
needed to ensure a unique streamfunction (see the discussions by
...
\cite{delSastre04} and are summarized in \autoref{tab:OceanParameters}.
...
\begin{equation}
  ...
  \label{eqn:Beta}
\end{equation}
and the parameters given in \autoref{tab:OceanParameters} we see that
\begin{equation*}
...
to Europe or Africa. {\color{red} The rates of convergence are much lower than
expected, however this likely is due to the facet that the domain is non-smooth
which is likely causing the solution to be in a space which is less regular than
the $H^6$ regularity \cite{Soan2010} assumed by the error analysis in \autoref{sec:SQGEErrors}.}
...
\begin{figure}
  \begin{center}
    ...
    \caption{Solution of SQGE on the Mediterranean with $Re = 5.27, Ro
